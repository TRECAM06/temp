...
hands = mpHands.Hands(max_num_hands=1, min_detection_confidence=0.3, min_tracking_confidence=1)
...
def populate_folder(image_counter):
    ...
    while(True):
        ...
        cv2.imwrite("training_data3/".format(type_of_picture)+"{}-{}.jpg".format(type_of_picture,image_counter), frame)
        image_counter += 1
    cap.release()
...
folderRes = 'training_data3' + '/' + folder + '.csv'
count = -1
